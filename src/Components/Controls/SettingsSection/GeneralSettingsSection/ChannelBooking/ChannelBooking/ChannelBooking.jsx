import React, { useContext, useEffect, useState } from 'react';
import { AuthContext } from '../../../../../Helpers/Context/AuthContext';
import { useData } from '../../../../../Helpers/Context/useData';
import DropdownActions from '../../../../../Shared/DropdownActions/DropdownActions';
import ReusableSection from '../../../../../Shared/ReusableSection/ReusableSection';
import { useForm } from 'react-hook-form';
import axios from 'axios';
import DeleteModal from '../../../../../Shared/DeleteModal/DeleteModal';
import { ToastContext } from '../../../../../Helpers/Context/ToastContext ';
import { useTranslation } from 'react-i18next';
import { Modal } from 'react-bootstrap';
import CreateChannelBooking from './CreateChannelBooking';

const ChannelBooking = () => {

  const { baseUrlPms, Headers } = useContext(AuthContext);
  const { showToast } = useContext(ToastContext);
  const { t } = useTranslation();



  // Fetching data from the API
  const { data, fetchData } = useData('channel_booking/all/', Headers);
 
  const [branchesTags, setBranchesTags] = useState([]);
  const [collectedId, setCollectedId] = useState([]);
  const { reset } = useForm({ mode: "all" });
  const [deleteModalState, setDeleteModalState] = useState({
    isOpen: false,
    channleId: null,
  });
  const [editModalState, setEditModalState] = useState({
    isOpen: false,
    rowData: null,
    isEditMode: true
  });


  // Close delete modal
  const closeModal = () => setDeleteModalState({ isOpen: false, channleId: null });

  // Handle delete modal open
  const handleDeleteModal = (id) => {
    setDeleteModalState({ isOpen: true, channleId: id });
  };


  // Function to open the edit modal with row data
  const handleEditModal = (row) => {


    const formattedData = {
      id: row.id,
      channel_booking_name_ar: row?.pms_channel_booking_ar?.channel_booking_name_ar || "",
      channel_booking_name_en: row?.pms_channel_booking_en?.channel_booking_name_en || "",
      channel_booking_percentage: row?.channel_booking_percentage || "",

    };
    setEditModalState({ isOpen: true, rowData: formattedData });
  };
  // Function to close the edit modal
  const closeEditModal = () => {
    setEditModalState({ isOpen: false, rowData: null });
    fetchData();

  };
  // Filter function to validate row data before rendering
  const filterFn = (row) => row?.pms_channel_booking_en?.channel_booking_name_en &&
    row?.pms_channel_booking_ar?.channel_booking_name_ar;
  // data of table
  const getColumns = (t) => [
    {
      name: t("englishName"),
      visible: true,
      selector: (row) => row?.pms_channel_booking_en?.channel_booking_name_en,
      sortable: true,
      reorder: true,
      cell: (row) => (
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="w-50">{row?.pms_channel_booking_en?.channel_booking_name_en}</div>
          <DropdownActions
            onDelete={() => handleDeleteModal(row.id)}
            onEdit={() => handleEditModal(row)}

          />
        </div>
      ),
    },

    {
      name: t("arabicName"),
      visible: true,
      selector: (row) => row?.pms_channel_booking_ar?.channel_booking_name_ar,
      sortable: true,
    },

    {
      name: t("channelBookingSection.percentage") + "%",
      visible: true,
      selector: (row) => row?.channel_booking_percentage,
      sortable: true,
    },
  ];
  const deleteChannleBooking = () => {
    const { channleId } = deleteModalState;
    if (!channleId) return;

    axios
      .delete(`${baseUrlPms}/channel_booking/${channleId}/delete/`, { headers: Headers })
      .then(() => {
        showToast("success", t("msg.removeChannelMsg"));
        closeModal();
        fetchData();
      })
      .catch((error) => {
        console.error("Error deleting room type:", error);
        // Add user feedback here, e.g., a toast message
      });
  };


  const fetchBranchesForSelectedChannelBooking = (channleId) => {
    axios.get(`${baseUrlPms}/channel_booking_branch/all/`, {
      params: {

        channel_booking_id: channleId
      },
      headers: Headers
    }).then((res) => {

      const branchesInfo = res.data.map((branch) => branch.branch_info.pms_branch_en.branch_name_en);
      const IdfochannleBookingandbranch = res.data.map((id) => id.id)
      setCollectedId(IdfochannleBookingandbranch);
      setBranchesTags(branchesInfo);
    }).catch((error) => {
      console.log(error);

    })
  }

  useEffect(() => {
    fetchBranchesForSelectedChannelBooking()
  }, [])
  return (
    <>
      {/* ReusableSection for rendering the table with provided configurations */}
      <ReusableSection
        data={data}
        filterFn={filterFn}
        getColumns={getColumns} />

      {deleteModalState.isOpen && (
        <DeleteModal
          show={deleteModalState.isOpen}
          closeModal={closeModal}
          fun={deleteChannleBooking}
        />
      )}
      {editModalState.isOpen && (
        <Modal show={editModalState.isOpen} onHide={closeEditModal} centered >
          <Modal.Header closeButton>
            <h4>
              Edit Channel Booking
            </h4>
          </Modal.Header>
          <Modal.Body>
            <CreateChannelBooking
              key={editModalState.rowData?.id || "new"}
              initialData={editModalState.rowData}
              onClose={closeEditModal}
              isEditMode={true}
              fetchBranchesForSelectedChannelBooking={fetchBranchesForSelectedChannelBooking}
              branchesTags={branchesTags}
              collectedId={collectedId}
              reset={reset}
              setCollectedId={setCollectedId}
            />
          </Modal.Body>
        </Modal>
      )}
    </>
  );
}

export default ChannelBooking