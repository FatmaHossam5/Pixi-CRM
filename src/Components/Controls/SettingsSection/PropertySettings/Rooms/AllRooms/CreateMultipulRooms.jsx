...
const CreateMultipulRooms = () => {
    ...
    const { branchs, handleHotelChange } = useBranch();
    ...
    const createRoom = async (data) => {
        ...
        for (let i = 0; i < generateNumber; i++) {
            const enNumber = Number(data.EnglishNameNumber) + i;
            const arNumber = Number(data.ArabicNameNumber) + i;
            let en, ar;
            if (data?.EnglishNameSuffix !== undefined || "" || null) {
                en = `${data.EnglishNamePrefix
                    }-${enNumber}${`- ${data?.EnglishNameSuffix}`}`;
            } else {
                en = `${data.EnglishNamePrefix}-${enNumber}`;
            }

            if (data?.ArabicNameSuffix !== undefined || "" || null) {
                ar = `${data.ArabicNamePrefix
                    }-${arNumber}${`-${data?.ArabicNameSuffix}`}`;
            } else {
                ar = `${data.ArabicNamePrefix}-${arNumber}`;
            }
            // First unit reference for room
            ...
            try {
                ...
                const roomId = roomData.data.data.unit_reference_id;

                const roomDataAr = await axios.post(
                ...
                const roomDataEn = await axios.post(
                ...
                const roomeDfaultPrice = await axios.post(
                ...
                // Loop to create multiple beds
                for (let i = 0; i < data.number_of_beds; i++) {
                    // Create unit reference for each bed
                    ...
                    // Create bed with the unit reference ID
                    const bedData = await axios.post(
                        `${baseUrlPms}/bed/store/`,
                        {
                            created_by: userId,
                            updated_by: userId,
                            room_id: roomId,
                            unit_reference_id: bedUnitReferenceId, // Use unit reference ID for bed
                        },
                        {
                            headers: Headers,
                        }
                ...
                if (
                    (roomData.status && roomDataAr.status && roomDataEn.status && roomeDfaultPrice.status == 200) ||
                    201
                ) {
...
export default CreateMultipulRooms