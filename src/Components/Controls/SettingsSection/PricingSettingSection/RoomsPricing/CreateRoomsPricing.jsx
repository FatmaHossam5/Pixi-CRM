...
const CreateRoomsPricing = () => {
  ...
  const createRoomPricing = async (data) => {
    console.log(data);

    const postPricingData = async (extraData) => {
      return await axios.post(
        `${baseUrlPms}/room_pricing_rule/store/`,
        {
          created_by: userId,
          updated_by: userId,
    ...
    try {
      ...
      await postPricingData({
        ...
        room_price: data.room_price_work_day,
      });
      if (data.workDays && data.workDays.length !== 0) {
        for (let index = 0; index < data.workDays.length; index++) {
          ...
          await postPricingData({
            ...
            special_day_branch_season_id: null,
            weekend_branch_season_ids: null,
            room_price: workday.room_price_work_day,
          });
        }
      }
      ...
      await postPricingData({
        ...
        weekend_branch_season_ids: null,
        room_price: data.room_price_special_day,
      });
      if (data.specialDays && data.specialDays.length !== 0) {
        for (let index = 0; index < data.specialDays.length; index++) {
          ...
          await postPricingData({
            ...
            weekday_branch_season_id: null,
            weekend_branch_season_ids: null,
            room_price: specialDay.room_price_special_day,
          });
        }
      }
      ...
      await postPricingData({
        ...
        room_price: data.room_price_week_end,
      });
      if (data.weekendDays && data.weekendDays.length !== 0) {
        for (let index = 0; index < data.weekendDays.length; index++) {
          ...
          await postPricingData({
            ...
            room_price: weekendDay.room_price_week_end,
          });
        }
      }
      ...
      //   for (let index = 0; index < data.weekendDays.length; index++) {
      //     const weekendDay = data.weekendDays[index];
      //     const weekendIds = allWeekendIds[index] || []; // Retrieve stored weekendIds for the section

      //     await postPricingData({
      //       special_day_branch_season_id: null,
      //       weekday_branch_season_id: null,
      //       weekend_branch_season_ids: weekendIds,
      //       room_price: weekendDay.room_price_week_end,
      //     });
      //   }
      // }

      // // If there is a main section (outside useFieldArray items)
      // if (data.room_price_week_end) {
      //   await postPricingData({
      //     special_day_branch_season_id: null,
      //     weekday_branch_season_id: null,
      //     weekend_branch_season_ids: allWeekendIds["main"] || [],
      //     room_price: data.room_price_week_end,
      //   });
      // }
    ...
    <>
      <div className="px-content mb-auto mt-3">
        <div className="px-card p-4 text-capitalize ">
            ...
            <div className="w-100 d-flex flex-wrap align-items-center">
              <div className="breadcrumbs-list mb-2 w-100">
                <Breadcrumb>
                  ...
                  <Breadcrumb.Item>
                    <Link to={"/dashboard/pricing"}>Rooms Pricing</Link>
                  </Breadcrumb.Item>
                  <Breadcrumb.Item active>Add New Room Price</Breadcrumb.Item>
                </Breadcrumb>
              </div>
          ...
          <form
            onSubmit={handleSubmit(createRoomPricing)}
...
export default CreateRoomsPricing;
