...
const BuildingsPricing = () => {
      ...
      const columns = [
        {
          ...
          selector: (row) => row?.building_info?.pms_building_en?.building_name_en,
          sortable: true,
          reorder: true,
          cell: (row) => (
            <div className="d-flex justify-content-between align-items-center w-100">
              <div className="w-50">
                {row?.building_info?.pms_building_en?.building_name_en}
              </div>
        ...
        {
          name: "Branch",
          selector: (row) =>  row?.building_info?.branch_info?.pms_branch_en?.branch_name_en,
          sortable: true,
        },
        {
          name: "Workdays Price",
          selector: (row) => `${row.building_price} $`
        },
        {
 ...
 const [buildingsPricing, setBuildingsPricing] = useState();
 const getBuildingsPricing = () => {
   axios
     .get(`${baseUrlPms}/building_pricing_rule/all/`, {
       headers: Headers,
     })
     .then((res) => {
       setBuildingsPricing(res.data);
     })
     .catch((error) => {
       console.log(error);
     });
    ...
    const filteredRows = buildingsPricing?.filter(
      (row) =>
        row?.building_info?.pms_building_en?.building_name_en &&
        row?.building_info?.pms_building_ar?.building_name_ar
    );
      ...
      >
        <div className="px-card">
          <div className="card-head d-flex p-4 align-items-center">
            ...
            <Link to="buildingPricing" className="px-btn px-blue-btn ms-3">
              create new building price
            </Link>
...
export default BuildingsPricing
