...
import CreateRooms from "../SettingsSection/PropertySettings/Rooms/AllRooms/CreateRooms";
...
import CreateMultipulRooms from "../SettingsSection/PropertySettings/Rooms/AllRooms/CreateMultipulRooms";
import CreateSingleRooms from "../SettingsSection/PropertySettings/Rooms/AllRooms/CreateSingleRooms";
...
const RoomsSection = () => {
  ...
  const [roomStatus, setRoomsStatus] = useState([]);
  const getRoomStatus = () => {
    axios
  ...
  const changeStatus = (roomId, statusId) => {
    axios
      .post(
        `${baseUrlPms}/room_roomstatus/store/`,
  ...
  const roomProfile = (id) => {
    navagite(`roomProfile/${id}`);
  };

  const filteredRows = rooms?.filter((row) => row?.pms_room_en?.room_name_en);
  const filteredRooms = rooms?.filter((room) => {
    const roomName = room?.pms_room_en?.room_name_en?.toLowerCase();
    const roomType =
      room?.room_type_info?.pms_room_type_en.room_type_name_en?.toLowerCase();

    const searchQuery = search.toLowerCase();

    // You can adjust this condition to include more fields or change the logic
    return roomName?.includes(searchQuery) || roomType?.includes(searchQuery);
  });

  const columns = useMemo(() => [
    {
      name: t("RoomSection.RoomNumber"),
      visible: true,
      selector: (row) => language === "ar" ? row?.pms_room_ar?.room_name_ar : row?.pms_room_en?.room_name_en,
      sortable: true,
      reorder: true,
      cell: (row) => (
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="w-50">{language === "ar" ? row?.pms_room_ar?.room_name_ar : row?.pms_room_en?.room_name_en}</div>
          <div className="dropdown">
            ...
            <ul className="dropdown-menu">
              <li>
                ...
                <Link className="dropdown-item" onClick={() => roomProfile(row.unit_reference_id)}>
                  {t("view")}
              ...
              <li>
                <a className="dropdown-item"> {t("RoomSection.roomMenu.checkout")}</a>
              </li>
              <li>
                <hr className="dropdown-divider w-80 m-auto opacity-75" />
              </li>
              <li>
                <a className="dropdown-item" >
                  {t("RoomSection.roomMenu.Blocked")}
                </a>
              </li>
              <li>
                <hr className="dropdown-divider w-80 m-auto opacity-75" />
    ...
    {
      name: t("RoomSection.type"),
      visible: true,
      selector: (row) => language === "ar" ? row?.room_type_info?.pms_room_type_ar.room_type_name_ar
        : row?.room_type_info?.pms_room_type_en.room_type_name_en,
      sortable: true,
    },
    {
      name: t("RoomSection.cleaningStatus"),
      visible: true,
      selector: (row) => (row.is_cleaned == false ? t("RoomSection.notCleaned") : t("RoomSection.cleaned")),
      sortable: true,
    },
    {
      name: t("RoomSection.maintenance"),
      visible: true,
      selector: (row) => "null",
      sortable: true,
    },
    {
      name: t("RoomSection.reservationStatus"),
      visible: true,
      selector: (row) => "null",
      sortable: true,
    },
    {
      name: t("RoomSection.guest"),
      visible: true,
      selector: (row) => "null",
      sortable: true,
    },
  ...
  const handleDelete = () => {
    try {
      const room = axios.delete(`${baseUrlPms}/room/${selectedRoom}/delete/`, {
        headers: Headers,
      });
      console.log(selectedRoom);

      const roomAr = axios.delete(
        `${baseUrlPms}/room_ar/${selectedRoom}/delete/`,
        {
          headers: Headers,
        }
      );
      ...
      const roomEn = axios.delete(
        `${baseUrlPms}/room_en/${selectedRoom}/delete/`,
        {
          headers: Headers,
        }
      );
      if ((room.status && roomEn.status && roomAr.status) === 200 || 201) {
        setModelState({
          status: "success",
      ...
      setModelState({
        status: "error",
        message:
          error.response?.data?.non_field_errors?.[0] || "you have an error",
      });
    } finally {
      getRooms();
    }
        ...
        <div className="px-card mt-4">
          <div className="card-head d-flex p-4 pb-0 flex-wrap align-items-center">
            <div className="w-100 d-flex flex-wrap align-items-center">
              ...
              <div className="px-dropdown-btn ms-auto">
                <button
              ...
              {/* <AddModal
                ...
                component={<CreateRooms getRooms={getRooms} />}
              /> */}
              <AddModal
                name="SingleRoom"
                showState={showState}
                handleClose={handleClose}
                title={`${t('createNew')} ${t("room")} `}
                // component={<CreateMultipulRooms getRooms={getRooms} />}
                component={<CreateSingleRooms getRooms={getRooms} />}
              // component={<CreateRooms getRooms={getRooms} />}
              />
            </div>
                ...
                <div className="row gx-0">
                  {filteredRooms &&
                    filteredRooms?.map((room) => {
                      return (
                        <>
                          <div className="col-12 col-md-10 col-lg-4 col-xxl-3">

                            <div
                              className="m-2 room-view-card"
                              style={{
                                borderColor:
                                  room?.room_room_status_info?.room_status_info
                                    ?.room_status_color,
                                borderStyle: "solid",
                                borderWidth: "1px",
                              }}
                            >
                              ...
                              <div className="view-card-head d-flex justify-content-between">
                                <div className=" px-3 py-2">
                                  <select
                                    ...
                                    onChange={(e) =>
                                      changeStatus(room.unit_reference_id, e.target.value)
                                    }
                                  >
                                    <option value="">{t("selectInput.choose") + " " + t("status")}</option>
                                    {roomStatus &&
                                      roomStatus.map((roomstatus) => {
                                        return (
                                          <option
                                            ...
                                            value={
                                              roomstatus.id
                                              // room?.room_room_status_info
                                              //   ?.room_status_info
                                              //   ?.pms_room_status_en
                                              //   .room_status_name_en == "Cleaned"
                                              //   ? room?.room_room_status_info
                                              //     ?.room_status_info.id
                                              //   : roomstatus.id
                                            }
                                          ...
                                          //   : roomstatus.id === (room?.room_room_status_info?.room_status_info?.id || roomStatus[11].id) }
                                          >
                                            {language === "ar" ?
                                              roomstatus?.pms_room_status_ar
                                                ?.room_status_name_ar :
                                              roomstatus?.pms_room_status_en
                                                ?.room_status_name_en
                                            }
                                          </option>
                                        ); FF
                              ...
                              <div className="view-card-body p-2 d-flex flex-column justify-content-center align-items-center">
                                <div className="room-name">
                                  <h5>{language === "ar" ? room.pms_room_ar?.room_name_ar : room.pms_room_en?.room_name_en}</h5>
                                </div>
                              </div>

                              <div className="view-card-footer py-2 container-fluid"
                                style={{ backgroundColor: room?.room_room_status_info?.room_status_info?.room_status_color }}>

                                <div className="row gx-0">
                                  <div className="col-4 gx-0">
                                    <div className="room-type vertical-separetor text-center">
                                      {
                                        language === "ar" ?
                                          room.room_type_info?.pms_room_type_ar
                                            .room_type_name_ar :
                                  ...
                                  <div className="col-4 gx-0">
                                    ...
                                    {/*  end test select */}
                                    <div className="room-cleanliness vertical-separetor text-center">

                                      {/* {room.is_cleaned == false
                                        ? "not cleaned"
                                        : "cleaned"} */}
                                      {language === "ar" ? room?.room_room_status_info
                                        ?.room_status_info?.pms_room_status_ar
                                        .room_status_name_ar || "null"
                                        :
                                        room?.room_room_status_info
                                          ?.room_status_info?.pms_room_status_en
                                          .room_status_name_en || "null"}

                                    </div>
                                  </div>
                                  <div className="col-4 gx-0">
                                    <div className="room-status text-center">
                                      {room.is_empty ? t('RoomSection.empty') : t('RoomSection.filled')}
                                    </div>
                                  </div>
                                </div>
                              </div>
