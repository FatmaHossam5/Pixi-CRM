...
const Lead = () => {
  ...
  const [viewMode, setViewMode] = useState(localStorage.getItem("lead_view_mode") || "table");
  ...
  useEffect(() => {
    localStorage.setItem("lead_view_mode", viewMode);
  }, [viewMode]);
  const statusStyles = {
    ...
    <>
      ...
      <div className="d-flex  gap-3 " style={{
        ...
        {stagesMeta.map((stage) => {
          const leadsInStage = leadData.filter((lead) => lead.stage === stage.name);
          return (
            <div
              key={stage.name}
  ...
  const taskOffcanvas = (
    <AddFormOffcanvas
      name="TaskForm"              // must match the string we pass to setShowState()
      showState={showState}
      handleClose={() => setShowState(null)}
      title="Add Task"
      formComponent={<CreateTask />} // This is your actual form for tasks
    />
  );
  ...
  return (
    <>
      ...
      <DynamicSectionForTabs
        data={leadData}
        fetchData={() => setLeadData()}
        createComponent={<CreateLead refetch={() => setLeadData()} />}
        translationKey="Lead"
   ...
   <AddFormOffcanvas
        ...
        formComponent={<CreateLead initialData={selectedLead} />}
      />
    </>
  );
};
